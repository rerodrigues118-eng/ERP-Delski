...
export const Route = createFileRoute("/p/$token")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Visualize especificações do projeto Delski via link exclusivo.",
      },
      ...
      <div className="min-h-screen grid place-items-center px-4">
        <div className="text-center">
          ...
          <p className="text-muted-foreground mt-2">
            Este link foi revogado ou o projeto não existe.
          </p>
  ...
  const onUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Array.from(files).forEach((f) =>
      addFile(project.id, {
        ...
        uploadedBy: freelancer?.name || "Freelancer",
      }),
    );
    ...
    <div className="min-h-screen bg-muted/30 py-10 px-4">
      <div className="max-w-3xl mx-auto space-y-4">
        <div className="flex items-center gap-2">
          <div className="grid h-8 w-8 place-items-center rounded-lg bg-brand text-brand-foreground font-bold">
            D
          </div>
        ...
        <Card>
          <CardHeader>
            ...
            <p className="text-sm text-muted-foreground">
              {SERVICE_LABEL[project.type]} · prazo{" "}
            ...
            {project.referenceLink && (
              <div>
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="text-brand text-sm break-all hover:underline"
                >
                  {project.referenceLink}
                </a>
              </div>
                ...
                <div key={f.id} className="flex items-center justify-between py-2">
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">
                      {(f.size / 1024).toFixed(1)} KB · {f.uploadedBy}
                    </div>
                  </div>

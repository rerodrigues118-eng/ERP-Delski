...
export const Route = createFileRoute("/app/risks")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Projetos parados, prazos estourados e gargalos automáticos.",
      },
...
export function RisksPage() {
  ...
  const analysis = useMemo(() => {
    ...
    for (const p of projects) {
      ...
      const last = p.lastStatusChangeAt
        ? new Date(p.lastStatusChangeAt).getTime()
    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Detecção automática de projetos que precisam de atenção agora.
        </p>
      </div>
      ...
      {totalRisks === 0 && (
        <Card>
          <CardContent className="py-12 text-center text-sm text-muted-foreground">
            Nenhum risco detectado. Operação saudável.
          </CardContent>
        </Card>
      )}
        ...
        <Card className="border-destructive/40">
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2 text-destructive">
              <Calendar className="h-4 w-4" /> Prazo estourado ({analysis.overdue.length})
            </CardTitle>
          </CardHeader>
                ...
                <div key={p.id} className="flex items-center justify-between py-2">
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">
                      {SERVICE_LABEL[p.type]} · {STATUS_LABEL[p.status]}
                    </div>
                  </div>
        ...
        <Card className="border-amber-500/40">
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2 text-amber-600">
              <Calendar className="h-4 w-4" /> Prazo próximo ≤ 3 dias ({analysis.dueSoon.length})
            </CardTitle>
          </CardHeader>
          <CardContent className="divide-y">
            {analysis.dueSoon.map((p) => (
              <div key={p.id} className="flex items-center justify-between py-2">
                <div>
                  <div className="font-medium">{p.client}</div>
                  <div className="text-xs text-muted-foreground">
                    Entrega em {new Date(p.deadline).toLocaleDateString("pt-BR")} ·{" "}
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2">
              <Clock className="h-4 w-4 text-amber-500" /> Parados no mesmo status há 5+ dias (
              {analysis.stuck.length})
            </CardTitle>
          </CardHeader>
          <CardContent className="divide-y">
              ...
              return (
                <div key={p.id} className="flex items-center justify-between py-2">
                  <div>
                    <div className="font-medium">{p.client}</div>
                    <div className="text-xs text-muted-foreground">
                      {STATUS_LABEL[p.status]} · {f?.name || "sem freela"}
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">
              Sem freelancer atribuído ({analysis.unassigned.length})
            </CardTitle>
          </CardHeader>
          <CardContent className="divide-y">

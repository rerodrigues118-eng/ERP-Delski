...
import { useDeleteWikiArticle, useSaveWikiArticle, useWikiArticles } from "@/hooks/useWikiArticles";
...
export const Route = createFileRoute("/app/wiki")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Base de conhecimento interna da Delski: prompts, padrões e boas práticas.",
      },
...
function WikiPage() {
  ...
  const { isCliente, isGestor: authIsGestor, loading: authLoading } = useAuth();
  const { data: wiki = [], isLoading } = useWikiArticles();
  const saveWikiMutation = useSaveWikiArticle();
  const deleteWikiMutation = useDeleteWikiArticle();
  const isGestor = authIsGestor;
  ...
  }, [isCliente, authLoading, navigate]);

  if (authLoading || isLoading) {
    return (
      <div className="p-16 text-center space-y-3">
        <div className="h-8 w-8 animate-spin rounded-full border-4 border-indigo-600 border-t-transparent mx-auto" />
        <p className="text-sm text-muted-foreground">Carregando wiki...</p>
      </div>
    );
  }
  ...
  const submit = () => {
    if (!editing?.title || !editing.content || !editing.category) {
      return toast.error("Preencha título, categoria e conteúdo.");
    }

    saveWikiMutation.mutate({
      id: editing.id,
      title: editing.title,
      ...
      <div className="flex items-end justify-between flex-wrap gap-2">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight flex items-center gap-2">
            <BookOpen className="h-6 w-6" /> Wiki & SOPs
          </h1>
          <p className="text-sm text-muted-foreground">
            Padrões, prompts validados e guias operacionais da Delski.
          </p>
        </div>
        {isGestor && (
          <Button onClick={() => setEditing({ title: "", category: "Geral", content: "" })}>
            ...
            <div className="space-y-1 max-h-[60vh] overflow-y-auto">
              {filtered.map((a) => (
                <button
                  ...
                  className={`w-full text-left rounded-md px-2 py-2 text-sm hover:bg-accent transition-colors ${current?.id === a.id ? "bg-accent" : ""}`}
                >
                  <div className="font-medium truncate">{a.title}</div>
                  <div className="flex items-center gap-2 mt-1">
                    <Badge variant="outline" className={catColor[a.category]}>
                      {a.category}
                    </Badge>
              ...
              {filtered.length === 0 && (
                <div className="text-xs text-muted-foreground text-center py-6">Nenhum artigo.</div>
              )}
            </div>
          </CardContent>
        </Card>
              ...
              <CardHeader className="flex flex-row items-start justify-between gap-2 space-y-0">
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <Badge variant="outline" className={catColor[current.category]}>
                      {current.category}
                    </Badge>
                    <span className="text-xs text-muted-foreground">
                      Atualizado em {new Date(current.updatedAt).toLocaleDateString("pt-BR")}
                    </span>
                  </div>
                </div>
                {isGestor && (
                  <div className="flex gap-1">
                    ...
                    <Button
                      ...
                      variant="ghost"
                      onClick={() => {
                        deleteWikiMutation.mutate(current.id);
                        setSelected(null);
                      }}
                    >
          ...
          <div className="grid gap-3">
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setEditing({ ...editing!, category: v as WikiArticle["category"] })
                }
              >
                <SelectTrigger>
                  <SelectValue />

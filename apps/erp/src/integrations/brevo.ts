...
export async function sendClientAccessInviteEmail(args: {
  ...
  const loginUrl =
    args.customLink ||
    (typeof window !== "undefined"
      ? `${window.location.origin}/portal/auth?email=${encodeURIComponent(args.to.email)}`
      : `http://localhost:8080/portal/auth?email=${encodeURIComponent(args.to.email)}`);

  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 24px; border: 1px solid #e5e7eb; border-radius: 12px; background-color: #ffffff;">
      <div style="text-align: center; margin-bottom: 24px;">
        <h2 style="color: #4f46e5; margin: 0; font-size: 24px;">Convite de Acesso ao Portal do Cliente — Delski</h2>
      </div>
      <p style="font-size: 16px; color: #374151; line-height: 1.5;">Olá <strong>${args.to.name}</strong>${args.companyName ? ` (${args.companyName})` : ""},</p>
      <p style="font-size: 15px; color: #4b5563; line-height: 1.5;">
        Sua conta de cliente foi registrada na plataforma Delski ERP. Clique no botão abaixo para confirmar seu e-mail e definir sua senha de acesso.
      </p>
      <div style="background-color: #f9fafb; padding: 16px; border-radius: 8px; margin: 20px 0; border: 1px solid #f3f4f6;">
        <p style="margin: 0; font-size: 14px; color: #6b7280;">Seu e-mail de acesso:</p>
        <p style="margin: 4px 0 0 0; font-size: 16px; font-weight: bold; color: #111827;">${args.to.email}</p>
      </div>
      <div style="text-align: center; margin: 28px 0;">
        <a href="${loginUrl}" style="display: inline-block; background-color: #4f46e5; color: #ffffff; font-weight: bold; font-size: 16px; text-decoration: none; padding: 14px 28px; border-radius: 8px; box-shadow: 0 4px 6px -1px rgba(79, 70, 229, 0.2);">
      ...
      <hr style="border: 0; border-top: 1px solid #e5e7eb; margin: 24px 0;"/>
      <p style="font-size: 12px; color: #9ca3af; text-align: center; margin: 0;">
        Delski ERP — Gestão de Projetos e Portal do Cliente
      </p>
    </div>
  `;

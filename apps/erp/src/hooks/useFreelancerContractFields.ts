...
export function checkRequiredDocumentsStatus(docs: FreelancerDocument[]): {
  ...
  if (!uploadedTypes.has("foto_rosto_3x4")) {
    missing.push("Foto do Rosto (tipo 3x4)");
  }
...
export function useUploadFreelancerDocument() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      // If document is foto_rosto_3x4, sync public URL to profiles.avatar_url
      if (documentType === "foto_rosto_3x4") {
        const { data: urlData } = supabase.storage
          .from("freelancer-documents")
        ...
        if (urlData?.publicUrl) {
          try {
            ...
            console.warn("Sync avatar_url fallback:", err);
          }
        }
      }

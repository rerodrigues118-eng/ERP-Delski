...
function ClientAuthPage() {
  const searchParams = useSearch({ from: "/portal/auth" });
  ...
  useEffect(() => {
    if (!authLoading && user) {
      if (isGestor || isFreelancer) {
        toast.info("Acesso interno detectado. Redirecionando para a área operacional.");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isFirstAccess) {
        ...
        // SignUp or set password for client invite
        ...
        if (loggedUserId) {
          ...
          if (userRole && userRole !== "cliente" && userRole !== "client") {
            ...
            toast.error(
              "Esta tela de acesso é exclusiva para Clientes. Utilize a página de login interna.",
            );
            navigate({ to: "/auth", replace: true });
            return;
          }
  ...
  return (
    <div className="min-h-screen bg-[#FAFAF9] text-stone-900 flex flex-col justify-between p-4 sm:p-8 font-sans">
      ...
      <footer className="max-w-md mx-auto w-full text-center pb-4 text-[11px] text-stone-400">
        © {new Date().getFullYear()} Delski — Todos os direitos reservados. Portal Exclusivo do
        Cliente.
      </footer>
    </div>
  );

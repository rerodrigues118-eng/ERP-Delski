...
function ClientPortal() {
  ...
  const approve = () => { addFeedback(project.id, "aprovado"); toast.success("Aprovação registrada."); };
  const requestChange = () => {
    ...
    <div className="min-h-screen bg-muted/30">
      <header className="border-b bg-card">
        <div className="max-w-4xl mx-auto flex items-center gap-3 px-6 py-4">
          <div className="grid h-9 w-9 place-items-center rounded-lg bg-brand text-brand-foreground font-bold">D</div>
          <div>
      ...
      <main className="max-w-4xl mx-auto px-6 py-8 space-y-6">
          ...
          <CardContent className="space-y-4">
            <div>
              <div className="flex justify-between text-xs text-muted-foreground mb-2">
                <span>Etapa atual: <strong className="text-foreground">{STATUS_LABEL[project.status]}</strong></span>
                <span>{progress}%</span>
              </div>
            ...
            <div className="grid grid-cols-5 gap-2">
              {STATUSES.map((s, i) => (
                <div key={s} className={`text-center text-xs p-2 rounded-md border ${i <= currentIdx ? "bg-brand/10 border-brand/30 text-foreground" : "text-muted-foreground"}`}>
                  {STATUS_LABEL[s]}
                </div>
              ))}
            </div>
        ...
        <Card>
          <CardHeader><CardTitle className="text-base">Escopo do projeto</CardTitle></CardHeader>
        ...
        {(project.driveLink || project.files.length > 0) && (
          <Card>
            ...
            <CardContent className="space-y-2">
              {project.driveLink && (
                <a href={project.driveLink} target="_blank" rel="noreferrer" className="flex items-center justify-between rounded-md border p-3 hover:bg-accent transition-colors">
                  <span className="text-sm">Pasta de arquivos (Google Drive)</span>
                  <ExternalLink className="h-4 w-4" />
                </a>
              )}
              {project.files.map((f) => (
                <a key={f.id} href={f.url} download={f.name} className="flex items-center justify-between rounded-md border p-3 hover:bg-accent transition-colors">
        ...
        {project.status === "Em Revisao" && (
          <Card className="border-brand/40">
            ...
            <CardContent className="space-y-3">
              <p className="text-sm text-muted-foreground">A entrega está pronta para revisão. Aprove ou peça ajustes:</p>
              <Textarea value={msg} onChange={(e) => setMsg(e.target.value)} placeholder="Descreva o ajuste (se necessário)..." rows={3} />
              <div className="flex gap-2">
                <Button onClick={approve} className="flex-1"><CheckCircle2 className="h-4 w-4" /> Aprovar entrega</Button>
                <Button variant="outline" onClick={requestChange} className="flex-1"><MessageSquareWarning className="h-4 w-4" /> Solicitar ajuste</Button>
              </div>
            </CardContent>
          </Card>
        )}
        ...
        {project.clientFeedback && project.clientFeedback.length > 0 && (
          <Card>
            <CardHeader><CardTitle className="text-base">Seu histórico de aprovações</CardTitle></CardHeader>
            <CardContent className="divide-y">
              {project.clientFeedback.slice().reverse().map((fb) => (
                <div key={fb.id} className="py-2">
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-xs text-muted-foreground">{new Date(fb.at).toLocaleString("pt-BR")}</span>
                  </div>
                  {fb.message && <p className="text-sm mt-1 text-muted-foreground">{fb.message}</p>}
                </div>
              ))}
            </CardContent>
          </Card>

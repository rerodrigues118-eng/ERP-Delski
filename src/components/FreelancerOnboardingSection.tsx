...
export function FreelancerOnboardingSection() {
  ...
  const handleUploadSigned = async (contractId: string, e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    ...
    try {
      ...
      console.error(err);
      toast.error(err?.message || "Falha ao enviar contrato assinado.");
    } finally {
  ...
  const hasAllRequiredAttached =
    hasIdentityAttached && hasResidenciaAttached && hasCpfAttached && hasAntecedentesAttached;
  ...
  const isLockedState =
    (documentsStatus === "em_analise" || documentsStatus === "aprovado") && !hasRejectedDocs;
      ...
      <Card className="border-border bg-card shadow-sm">
        <CardHeader className="pb-3">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            ...
            {documentsStatus === "em_analise" && !hasRejectedDocs && (
          ...
          {loadingContracts ? (
            ...
            <div className="space-y-4">
              {generatedContracts.map((contract: any) => {
                ...
                const isUploadingThis = uploadingContractId === contract.id;
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        {contract.docx_path && (
                          <Button variant="outline" size="sm" asChild className="gap-1.5 text-xs">
                            <a href={contract.docx_path} target="_blank" rel="noreferrer">
                              <Download className="h-3.5 w-3.5 text-indigo-500" /> Baixar Minuta
                            </a>
                          </Button>
                        )}
                        ...
                        {contract.signed_docx_path ? (
                          ...
                          >
                            <a href={contract.signed_docx_path} target="_blank" rel="noreferrer">
                              <FileCheck className="h-3.5 w-3.5" /> Ver Assinado
                            </a>
                          </Button>
                        ) : (
                          <Label className="cursor-pointer">
                            <span className="inline-flex items-center gap-1.5 text-xs font-medium bg-emerald-600 text-white hover:bg-emerald-700 h-8 px-3 rounded-md transition-colors shadow-sm">
                              {isUploadingThis ? (
                                <>
                                ...
                                <>
                                  <Upload className="h-3.5 w-3.5" /> Enviar Contrato Assinado (PDF)
                                </>
                              )}
                            </span>
                            <Input
                              ...
                              onChange={(e) => handleUploadSigned(contract.id, e)}
                            />
                          </Label>
                        )}
                      </div>
